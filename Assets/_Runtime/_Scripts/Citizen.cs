...
public class Citizen : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (direction.magnitude >= 0.1f)
        {
            ...
            movDir += new Vector3(0, -20, 0);
        
        if (!isGrabbed)
        {
            control.Move(movDir.normalized * 5 * 20f* Time.deltaTime);
        }
        else
        {
    ...
    public void SpreadDisease()
    {
        if (selectedSprite == 5)
        {
            ...
            {
                GiveMasks.score -= 1;
                Instantiate(virus, new Vector3(this.transform.position.x, this.transform.position.y, this.transform.position.z), Quaternion.identity);
                fx.PlayOneShot(sick);
            }
        }
    }
